//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Web.Model
{
    using System;
    using System.Collections.Generic;
    
    public partial class view_NEWS_LISTDETAIL
    {
        public int CreatedBy { get; set; }
        public int CategoryId { get; set; }
        public string NguoiTao { get; set; }
        public int ID { get; set; }
        public string Title { get; set; }
        public Nullable<int> Status { get; set; }
        public string FeautureImage { get; set; }
        public string LangCode { get; set; }
        public System.DateTime CreatedDate { get; set; }
        public string DisplayZone { get; set; }
        public Nullable<int> PageElementId { get; set; }
        public Nullable<System.DateTime> ModifyDate { get; set; }
        public string Author { get; set; }
        public Nullable<byte> NoiBo { get; set; }
        public Nullable<byte> IsHot { get; set; }
        public Nullable<byte> IsSlide { get; set; }
        public Nullable<byte> IsNew { get; set; }
        public string CatName { get; set; }
        public string CatImage { get; set; }
        public string CatImage2 { get; set; }
        public string Description { get; set; }
        public string CropImage { get; set; }
        public string MorePageId { get; set; }
        public string MoreCategoryId { get; set; }
        public string MoreDisplayZone { get; set; }
        public Nullable<byte> IsGetNews { get; set; }
        public Nullable<byte> AllowComment { get; set; }
        public long ViewCount { get; set; }
        public Nullable<int> ModifyBy { get; set; }
        public string Tags { get; set; }
        public string KeyWords { get; set; }
        public Nullable<byte> IsRightSlide { get; set; }
        public string Details { get; set; }
    }
}
